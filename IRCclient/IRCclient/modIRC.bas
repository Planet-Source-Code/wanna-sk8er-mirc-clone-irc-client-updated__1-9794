...
Public ChatWindow() As New frmChat
Public ChatWindowName() As String
...
Public Sub ParseMsg(strHost As String, strTrigger As String, strMiddle As String, strMsg As String)
    ...
    Select Case UCase(strTrigger)
        ...
        Case "NOTICE"
            ...
            'Call DoColor(frmStatus.rtfStatus, "-" & strNick & "- " & strMsg & vbCrLf)
        ...
        Case "JOIN"
            ...
            If InStr(strHost, "!") Then
                ...
                If LCase(strNick) = LCase(MyNick) Then
                    ...
                    ChangeCaption LCase(parms), parms
        ...
        Case "PRIVMSG"
            ...
            'channel's private message
                ...
                Else
                    LogTextToHwnd LCase(strChannel), "<" & strNick & "> " & strText, Channel_hWnd, udtColor.colorAction, True
                End If
            Else
            ...
            End If
               ...
               Case "DCC"
                If UCase(word(2)) = UCase("CHAT") Then
                    If UCase(word(3)) = UCase("CHAT") Then
                        Call DoColor(frmStatus.rtfStatus, "4* DCC Chat request from " & username & " on his port " & word(5))
...
Public Sub AddTaskbar(xCaption As String, Image As Integer)
    frmMain.StatusBar.Panels.Add (frmMain.StatusBar.Panels.Count + 1), xCaption, xCaption
    frmMain.StatusBar.Panels.Item((frmMain.StatusBar.Panels.Count)).AutoSize = sbrContents
    frmMain.StatusBar.Panels.Item((frmMain.StatusBar.Panels.Count)).Bevel = sbrInset
    Select Case Image
        Case 1 'CHANNEL
            frmMain.StatusBar.Panels.Item((frmMain.StatusBar.Panels.Count)).Picture = frmMain.imgChannel.ListImages(1).Picture
    End Select
End Sub
Public Sub RemoveTaskbar(xCaption As String)
    Dim i As Integer
    
    For i = 1 To frmMain.StatusBar.Panels.Count
        'MsgBox frmMain.StatusBar.Panels.Item(i).Key & " --- " & xCaption
        If LCase(frmMain.StatusBar.Panels.Item(i).Key) = LCase(strChannel) & parms Then
            frmMain.StatusBar.Panels.Remove (i)
            Exit For
        End If
    Next i
End Sub
